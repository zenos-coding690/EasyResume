...
// --- Mock Data ---
const PRO_TEMPLATES = Array.from({ length: 12 }).map((_, i) => ({
  id: `pro-${i + 1}`,
  name: `Modèle Executive ${i + 1}`,
  category: 'Professionnel',
  color: ['bg-slate-800', 'bg-blue-600', 'bg-indigo-600', 'bg-emerald-600'][i % 4],
  image: `/images/templates/pro-${i + 1}.png`
}));

const CANADIAN_TEMPLATES = [
  {
    id: 'can-1',
    name: 'Format Canadien 1',
    category: 'Canada',
    color: 'bg-red-600',
    image: '/images/templates/can-1.jpg'
  },
  {
    id: 'can-2',
    name: 'Format Canadien 2',
    category: 'Canada',
    color: 'bg-red-600',
    image: '/images/templates/can-2.1.jpg'
  },
  {
    id: 'can-3',
    name: 'Format Canadien 3',
    category: 'Canada',
    color: 'bg-red-600',
    image: '/images/templates/can-3.1.jpg'
  }
];
...
function TemplateGrid({ 
  ...
  title: string, 
  templates: typeof PRO_TEMPLATES, 
  delay: number 
}) {
  const { t } = useLanguage();
    ...
    <ScrollReveal delay={delay} duration={700} direction="up" className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-extrabold text-slate-800 tracking-tight flex items-center gap-2">
          {title === t('sectionCanadianCV') && <span className="text-red-500">🍁</span>}
          {title}
        </h2>
      </div>
        ...
        {templates.map((tpl) => (
          ...
          >
            ...
            <div className="aspect-[1/1.4] bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-3 sm:p-5 relative overflow-hidden transition-all duration-500 group-hover:shadow-[0_20px_40px_rgb(16,98,254,0.12)] group-hover:-translate-y-2">
              ...
              {/* CSS Art: Simulated CV Layout (Fallback) */}
              <div className="flex flex-col h-full opacity-60 group-hover:opacity-100 transition-opacity duration-300 relative z-0">
                ...
                <div className="flex items-center space-x-2 sm:space-x-3 mb-4 sm:mb-6">
                  <div className={cn("w-6 h-6 sm:w-10 sm:h-10 rounded-full shrink-0", tpl.color)} />
                  <div className="space-y-1.5 sm:space-y-2 flex-1">
                    <div className="h-2 sm:h-2.5 bg-slate-200 rounded w-3/4" />
                    <div className="h-1 sm:h-1.5 bg-slate-100 rounded w-1/2" />
                  </div>
                </div>
                {/* Body (2 columns) */}
                <div className="flex gap-2 sm:gap-3 flex-1">
                  <div className="w-1/3 space-y-2 sm:space-y-3">
                  ...
                  <div className="w-2/3 space-y-2 sm:space-y-3">
                    <div className="h-1.5 sm:h-2 bg-slate-200 rounded w-1/3 mb-1 sm:mb-1.5" />
                    <div className="h-1 sm:h-1.5 bg-slate-100 rounded w-full" />
                    <div className="h-1 sm:h-1.5 bg-slate-100 rounded w-11/12" />
                    <div className="h-1 sm:h-1.5 bg-slate-100 rounded w-full" />
                    <div className="h-1.5 sm:h-2 bg-slate-200 rounded w-1/3 mt-3 sm:mt-4 mb-1 sm:mb-1.5" />
                    <div className="h-1 sm:h-1.5 bg-slate-100 rounded w-full" />
              ...
              <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center p-2 sm:p-4 space-y-2 sm:space-y-3 z-10">
                <Button 
                  ...
                  className="w-full bg-[#1062FE] hover:bg-blue-600 text-white rounded-lg sm:rounded-xl shadow-lg border-none flex items-center justify-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs py-1.5 sm:py-2 px-1 sm:px-3 h-auto"
                >
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 select-none pb-12">
      ...
      <div className="space-y-16">
        <TemplateGrid 
          title={t('sectionProCV')} 
          templates={PRO_TEMPLATES} 
          delay={200} 
        />
        
        <TemplateGrid 
          title={t('sectionCanadianCV')} 
          templates={CANADIAN_TEMPLATES} 
          delay={300} 
        />
      </div>
